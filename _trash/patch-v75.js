...
(function() {
  ...
  LeaderboardEngine.render = function() {
    ...
    board.forEach(function(entry, i) {
      html += '<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;background:rgba(255,255,255,0.03);border-radius:12px;border:1px solid ' + (i === 0 ? '#ffd700' : i === 1 ? '#c0c0c0' : i === 2 ? '#cd7f32' : 'rgba(255,255,255,0.1)') + '"><div style="font-size:1.5rem;font-weight:800;color:' + (i === 0 ? '#ffd700' : i === 1 ? '#c0c0c0' : i === 2 ? '#cd7f32' : 'var(--text-secondary)') + ';width:32px;text-align:center;">' + (i + 1) + '</div><div style="flex:1"><div style="font-size:1rem;font-weight:700;color:rgba(255,255,255,0.9);">' + _h(entry.name) + '</div><div style="font-size:0.75rem;color:var(--text-dim);">' + _h(entry.date) + '</div></div><div style="text-align:right;"><div style="font-size:1.2rem;font-weight:800;color:var(--cyber-blue);">' + entry.score + '分</div><div style="font-size:0.75rem;color:var(--text-secondary);">🔥' + entry.combo + 'x 🎯' + entry.accuracy + '%</div></div></div>';
    });
    html += '</div>';
    el.innerHTML = html;
  };
